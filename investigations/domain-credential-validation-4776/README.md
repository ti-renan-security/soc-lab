...
indicates that the username is valid but the supplied password is incorrect.
...
The initial event matched the default Wazuh rule:
...
The custom rule inherits from Wazuh rule 60104 and adds specific conditions for Windows Event ID 4776 and the incorrect-password status code.
...
MITRE ATT&CK T1110 covers techniques where adversaries attempt to gain access by systematically guessing passwords or credentials.
...
The detailed Wazuh event confirmed:
